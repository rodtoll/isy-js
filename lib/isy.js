...
exports.Families = Families_1.Families;
...
class ISY {
    ...
    loadDevices(obj) {
        ...
        for (const device of obj.nodes.node) {
            ...
            const d = DeviceFactory_1.DeviceFactory.getDeviceDetails(device.family, device.type);
            if (d.class) {
    ...
    initialize(initializeCompleted) {
        ...
        this.loadConfig().then(that.loadNodes, (reason) => that.handleInitializeError('Loading Config', reason)).then(that.refreshStatuses, (reason) => that.handleInitializeError('Loading Nodes', reason)).then(() => {
