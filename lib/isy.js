...
class ISY {
    ...
    callISY(url) {
        return _utils.getAsync.call(this, `${this.protocol}://${this.address}/rest/${url}/`, this.restlerOptions);
    }
    ...
    getDeviceTypeBasedOnISYTable(deviceNode) {
        ...
        if (familyId === _isyconstants.Families.insteon) {
            ...
            return this.buildDeviceInfoRecord(isyType, "Unknown", _isyconstants.DeviceTypes.unknown);
        }
    }
    ...
    getNodeDetail(device, callback) {
        ...
        (0, _restler.get)(`${this.protocol}://${this.address}/rest/nodes/${device.address}/`, this.restlerOptions).on('complete', result => {
    ...
    initialize(initializeCompleted) {
        ...
        this.loadConfig().then(this.loadNodes.bind(this)).then(() => this.refreshStatuses().then(() => {
            this.loadVariables(_isyconstants.VariableTypes.integer, () => {
                this.loadVariables(_isyconstants.VariableTypes.state, () => {
                    if (this.elkEnabled) {
                        (0, _restler.get)(`${this.protocol}://${that.address}/rest/elk/get/topology`, options).on('complete', (result, response) => {
                            if (that.checkForFailure(response)) {
