...
class ISY {
    ...
    async callISY(url) {
        ...
        const p = await utils_1.getAsync(url, this.restlerOptions);
        return p;
    }
                    ...
                    if (this.elkEnabled) {
                        restler_1.get(`${this.protocol}://${that.address}/rest/elk/get/topology`, options).on('complete', (result, response) => {
                            if (that.checkForFailure(response)) {
                                ...
                                restler_1.get(that.protocol +
                                    '://' +
                                    that.address +
                                    '/rest/elk/get/status', options).on('complete', (result, response) => {
                                    if (that.checkForFailure(response)) {
