...
class ISYDevice extends isynode_1.ISYNode {
    ...
    async updateProperty(propertyName, value) {
        ...
        this.logger('Updating property ' +
            isy_1.Controls[propertyName].label +
            '. incoming value: ' +
            value +
            ' outgoing value: ' +
            val);
    ...
    async refresh() {
        ...
        if (Array.isArray(node.property)) {
            for (const prop of node.property) {
                ...
                device.logger(`Property ${isy_1.Controls[prop.id].label} (${prop.id}) initialized to: ${device[prop.id]} (${device.formatted[prop.id]})`);
            }
        }
        else {
            ...
            device.logger(`Property ${isy_1.Controls[node.property.id].label} (${node.property.id}) initialized to: ${device[node.property.id]} (${device.formatted[node.property.id]})`);
        }
    }
