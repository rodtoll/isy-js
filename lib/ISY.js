...
class ISY {
    constructor(config, logger) {
        ...
        this.wsprotocol = 'ws';
        this.elkEnabled = (_a = config.elkEnabled, (_a !== null && _a !== void 0 ? _a : true));
    ...
    loadVariables(type, done) {
        const that = this;
        restler_1.get(`${that.protocol}://${that.address}/rest/vars/definitions/${type}`, this.restlerOptions).on('complete', (result, response) => {
            if (that.checkForFailure(response)) {
                ...
                restler_1.get(`${that.protocol}://${that.address}/rest/vars/get/${type}`, this.restlerOptions).on('complete', (result, response) => {
                    if (that.checkForFailure(response)) {
    ...
    handleInitializeError(step, reason) {
